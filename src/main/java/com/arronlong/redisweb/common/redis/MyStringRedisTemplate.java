...
public class MyStringRedisTemplate extends MyRedisTemplate<String, String> {
	...
	public MyStringRedisTemplate() {
		RedisSerializerSetting redisSerializerSetting = WebRuntimeContextHolder.getRuntimeContext().getRedisSerializerSetting();
		if(redisSerializerSetting!=null) {
			//使用上下文中保存的当前配置来完成初始化
			redisSerializerSetting.builder().setRedisTemplate(this).build();
		}else {
